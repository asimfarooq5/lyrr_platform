...
class ReaderController extends ChangeNotifier {
  ...
  Future<void> loadBookData() async {
    ...
    try {
      ...
      _syncWordsMap.clear();
      for (var sw in syncData) {
        _syncWordsMap[sw.id] = sw;
      }
  ...
  void triggerAutoScroll(
    ...
    for (var pos in positions) {
      if (pos.index == targetFlatIndex) {
        if (pos.itemLeadingEdge >= 0.05 && pos.itemTrailingEdge <= 0.95) {
          isVisible = true;
        }
        break;
      }
  ...
  void increaseFontSize() {
    _fontSize = (_fontSize + 2.0).clamp(12.0, 36.0);
    notifyListeners();
  }
